...
<body>
    ...
    <div class="form-container">
        <div class="header">
            ...
            <p>رقم الملف: <strong>{{ $orphan->file_number }}</strong></p>
        </div>

        <div class="photo-box">
            @if($orphan->photo)
                <img src="{{ asset('storage/' . $orphan->photo) }}">
            @else
        ...
        <div style="width: 73%;">
            ...
            <table class="data-table">
                <tr><td class="label">اسم اليتيم:</td><td class="value">{{ $orphan->name }}</td></tr>
                <tr><td class="label">الجنس / الجنسية:</td><td class="value">{{ $orphan->gender == 'male' ? 'ذكر' : 'أنثى' }} / {{ $orphan->nationality }}</td></tr>
                <tr><td class="label">العمر ومحل الميلاد:</td><td class="value">{{ \Carbon\Carbon::parse($orphan->birth_date)->age }} سنة - ({{ $orphan->birth_place }})</td></tr>
                <tr><td class="label">العنوان الحالي:</td><td class="value">{{ $orphan->address_gov }} - {{ $orphan->address_dist }} - {{ $orphan->address_village }}</td></tr>
            </table>

            <div class="section-title">ثانياً: التعليم والصحة</div>
            <table class="data-table">
                <tr><td class="label">الحالة التعليمية:</td><td class="value">
                    @php $edu = ['studying'=>'يدرس', 'stopped'=>'منقطع', 'not_enrolled'=>'غير ملتحق']; @endphp
                    {{ $edu[$orphan->education_status] ?? '---' }} | الصف: {{ $orphan->academic_level }}
                </td></tr>
                <tr><td class="label">المدرسة / الهاتف:</td><td class="value">{{ $orphan->school_name }} ({{ $orphan->school_phone }})</td></tr>
                <tr><td class="label">الحالة الصحية:</td><td class="value">{{ $orphan->health_status ?? 'سليم' }}</td></tr>
                <tr><td class="label">حفظ القرآن:</td><td class="value">{{ $orphan->quran_memorization ?? '---' }}</td></tr>
            </table>
        </div>

        <div class="section-title">ثالثاً: بيانات الأسرة (الوالدين)</div>
        <table class="data-table">
            <tr><td class="label">وفاة الوالد:</td><td class="value">السبب: {{ $orphan->father_death_cause }} | التاريخ: {{ $orphan->father_death_date }}</td></tr>
            <tr><td class="label">اسم الأم وحالتها:</td><td class="value">{{ $orphan->mother_name }} ({{ $orphan->mother_status == 'alive' ? 'على قيد الحياة' : 'متوفية' }})</td></tr>
        </table>

        <div class="section-title">رابعاً: بيانات المعيل</div>
        <table class="data-table">
            <tr><td class="label">اسم المعيل:</td><td class="value">{{ $orphan->guardian_name }} (صلة القرابة: {{ $orphan->guardian_relation }})</td></tr>
            <tr><td class="label">الهوية والهاتف:</td><td class="value">رقم البطاقة: {{ $orphan->guardian_card_id }} | الهاتف: {{ $orphan->guardian_phone }}</td></tr>
        </table>

        <div class="section-title">خامساً: النتيجة المدرسية لليتيم</div>
        <div class="result-section">
            @if($orphan->academic_result)
                <img src="{{ asset('storage/' . $orphan->academic_result) }}" class="result-image">
            @else
        ...
        <div class="thanks-section">
            @if($orphan->thank_you_letter)
                ...
                <img src="{{ asset('storage/' . $orphan->thank_you_letter) }}" class="thanks-image">
            @else
                <div style="padding: 25px; color: #999;">
    ...
    <div style="width: 100%; margin-top: 35px; padding-top: 15px; border-top: 2px solid #1a237e; text-align: center;">
        <div style="font-size: 12px; font-weight: bold; color: #333; line-height: 1.8;">
            ...
            @if(!empty($settings->phone)) <span style="display: inline-block; margin-right: 12px;">| هاتف: {{ $settings->phone }}</span> @endif
            @if(!empty($settings->email)) <span style="display: inline-block; margin-right: 12px;">| البريد: {{ $settings->email }}</span> @endif
            @if(!empty($settings->website)) <span style="display: inline-block; margin-right: 12px;">| الموقع: {{ $settings->website }}</span> @endif
            @if(!empty($settings->address)) <span style="display: inline-block; margin-right: 12px;">| العنوان: {{ $settings->address }}</span> @endif
        </div>
    </div>
</body>
</html>