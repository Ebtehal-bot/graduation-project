...
<head>
    <meta charset="UTF-8">
    <title>@yield('title', 'تقرير') - {{ $sysName ?? \App\Models\Setting::getValue('site_name', 'منصة كفيل لرعاية وكفالة الأيتام') }}</title>
    <style>
...
<body>
    ...
    <div class="header">
        @if (!empty($sysLogo))
            <img src="{{ asset('storage/' . $sysLogo) }}" alt="{{ $sysName }}" style="max-height: 55px; margin-bottom: 8px;">
        @endif
        <h1>{{ $sysName }}</h1>
        <h3>{{ $org }}</h3>
        <hr style="border: 1px solid #1a237e; margin: 12px 0;">
        <h2>@yield('report_title')</h2>
        <div class="report-meta">@yield('subtitle')</div>
    </div>
        ...
        <table>
            <tr>
                <td>
                    ...
                    @if(!empty($settings->phone)) <span class="sep">| هاتف: {{ $settings->phone }}</span> @endif
                    @if(!empty($settings->email)) <span class="sep">| البريد: {{ $settings->email }}</span> @endif
                    @if(!empty($settings->website)) <span class="sep">| الموقع: {{ $settings->website }}</span> @endif
                    @if(!empty($settings->address)) <span class="sep">| العنوان: {{ $settings->address }}</span> @endif
                </td>
            </tr>
        </table>
    </div>
