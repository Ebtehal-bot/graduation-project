...
<head>
    <meta charset="UTF-8">
    <title>استمارة تسجيل يتيم - {{ $orphan->name }}</title>
    <style>
    ...
    <div class="form-container">
        ...
        <div class="header">
            ...
            <table class="form-meta">
                <tr>
                    <td style="width: 50%;">
                        رقم الاستمارة:
                        <span class="meta-val">{{ $orphan->file_number ?? '______________' }}</span>
                    </td>
                    <td style="width: 50%;">
                        تاريخ الاستمارة:
                        <span class="meta-val">{{ now()->format('Y/m/d') }}</span>
                    </td>
                </tr>
            </table>
        </div>
        ...
        <div class="section clearfix">
            ...
            <table class="fields-table">
                <tr>
                    ...
                    <td class="fv">{{ $orphan->name ?? $na }}</td>
                    <td class="fl">الجنس / الجنسية:</td>
                    <td class="fv">
                        {{ isset($orphan->gender) ? ($orphan->gender == 'male' ? 'ذكر' : 'أنثى') : $na }}
                        /
                        {{ $orphan->nationality ?? $na }}
                    </td>
                </tr>
                <tr>
                    ...
                    <td class="fv">
                        @if($orphan->birth_date)
                            {{ \Carbon\Carbon::parse($orphan->birth_date)->age }} سنة
                        @else
                            {{ $na }}
                        @endif
                    ...
                    <td class="fv">
                        {{ $orphan->address_gov ?? $na }}
                        @if($orphan->address_dist) - {{ $orphan->address_dist }} @endif
                        @if($orphan->address_village) - {{ $orphan->address_village }} @endif
                    </td>
                </tr>
            </table>
        </div>
        ...
        <div class="section">
            ...
            <table class="fields-table">
                <tr>
                    ...
                    <td class="fv">{{ isset($orphan->education_status) ? ($eduMap[$orphan->education_status] ?? $orphan->education_status) : $na }}</td>
                    <td class="fl">الصف الدراسي:</td>
                    <td class="fv">{{ $orphan->academic_level ?? $na }}</td>
                </tr>
                <tr>
                    <td class="fl">المدرسة:</td>
                    <td class="fv">{{ $orphan->school_name ?? $na }}</td>
                    <td class="fl">هاتف المدرسة:</td>
                    <td class="fv">{{ $orphan->school_phone ?? $na }}</td>
                </tr>
                <tr>
                    <td class="fl">الحالة الصحية:</td>
                    <td class="fv" colspan="3">{{ $orphan->health_status ?? $na }}</td>
                </tr>
            </table>
        </div>
        ...
        <div class="section">
            ...
            <table class="fields-table">
                <tr>
                    ...
                    <td class="fv">
                        @if($orphan->father_death_cause || $orphan->father_death_date)
                            متوفى
                        @else
                            {{ $na }}
                        @endif
                    ...
                    <td class="fv">{{ $orphan->father_death_cause ?? $na }}</td>
                </tr>
                <tr>
                    <td class="fl">تاريخ الوفاة:</td>
                    <td class="fv">{{ $orphan->father_death_date ?? $na }}</td>
                    <td class="fl">اسم الأم:</td>
                    <td class="fv">{{ $orphan->mother_name ?? $na }}</td>
                </tr>
                <tr>
                    <td class="fl">حالة الأم:</td>
                    <td class="fv" colspan="3">
                        @if($orphan->mother_status == 'alive')
                            على قيد الحياة
                        @elseif($orphan->mother_status == 'deceased')
                            متوفية
                        @else
                            {{ $na }}
                        @endif
        ...
        <div class="section">
            ...
            <table class="fields-table">
                <tr>
                    ...
                    <td class="fv">{{ $orphan->guardian_name ?? $na }}</td>
                    <td class="fl">صلة القرابة:</td>
                    <td class="fv">{{ $orphan->guardian_relation ?? $na }}</td>
                </tr>
                <tr>
                    <td class="fl">رقم البطاقة:</td>
                    <td class="fv">{{ $orphan->guardian_card_id ?? $na }}</td>
                    <td class="fl">رقم الهاتف:</td>
                    <td class="fv">{{ $orphan->guardian_phone ?? $na }}</td>
                </tr>
            </table>
        </div>
        ...
        <div class="section">
            ...
            <div class="notes-area">
                @if($orphan->talents || $orphan->quran_memorization)
                    @if($orphan->talents)<div><strong>المواهب:</strong> {{ $orphan->talents }}</div>@endif
                    @if($orphan->quran_memorization)<div><strong>حفظ القرآن:</strong> {{ $orphan->quran_memorization }}</div>@endif
                @else
                    {{ $na }}
                @endif
            </div>
